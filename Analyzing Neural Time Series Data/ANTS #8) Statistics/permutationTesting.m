...
% load in data
load v1_laminar.mat
% 16(electrodes) x 1527(time points) x 200(trials)
...
zval = abs(norminv(pval)); % one-tailed normal, but why??
...
% 1000(permutations) * 50(frequency) * 141(downsampled time)
...
% 50 * 141 * 400(first 200 are from channels 5, second 200 are from channels 5)
...
% diffmap is real data, mean_h0 is simulating distribution's parameter, if
% there's no difference, swaping the trials in conditions makes no difference
% mean_h0 represents the diff difference the null
% (data - fluctuation) / normalize by variation
