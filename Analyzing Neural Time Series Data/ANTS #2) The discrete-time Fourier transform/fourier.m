...
N       = length(signal);    % length of sequence(num of points)
fourierTime = ((1:N)-1)/N;   % "time" used for sine waves(from 0 to 1, N points)
...
frequencies = linspace(0,nyquist,floor(N/2)+1); % freq can be measured in this sample rate
...
fourierCoefs = fourierCoefs / N; % computation involves N time points


figure(1), clf
subplot(221)
plot(real(exp( -2*pi*1i*(10).*fourierTime ))) % magicnum 10 is one of the sine wave
