...
srate = EEG.srate; % in hz, wavelet should has the same srate as in data
...
% now create complex Morlet wavelet
...
cmwX = fft(cmw,nConv); % if length of nConv > cmw, matlab zero padding
cmwX = cmwX ./ max(cmwX); % amplitude scale
...
length(conv_res)  % length of result of convolution
EEG.pnts  % length of EEG data points
