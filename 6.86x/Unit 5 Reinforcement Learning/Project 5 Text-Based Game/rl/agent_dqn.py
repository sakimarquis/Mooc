...
def epsilon_greedy(state_vector, epsilon):
    ...
    if np.random.random() > epsilon:
        ...
        action_index = torch.argmax(q_values_action_next)
        object_index = torch.argmax(q_values_object_next)
    else:
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    if not terminal:
        target_y = torch.FloatTensor([reward + GAMMA * maxq_next])
    else:
        target_y = torch.FloatTensor([reward])

    loss = F.mse_loss(q_value.requires_grad_(True), target_y.requires_grad_(True))
